...
'name' => __('Free Widget Configurator', 'reviews-widgets-for-yelp')
];
if ($this->is_noreg_linked()) {
$tabs[] = [
'place' => 'left',
'slug' => 'my-reviews',
'name' => __('My reviews', 'reviews-widgets-for-yelp')
];
}
$tabs[] = [
'place' => 'left',
'slug' => 'get-reviews',
'name' => __('Get Reviews', 'reviews-widgets-for-yelp')
];
$tabs[] = [
'place' => 'left',
'slug' => 'rate-us',
'name' => __('Rate Us', 'reviews-widgets-for-yelp')
];
if (!$this->is_trustindex_connected()) {
$tabs[] = [
'place' => 'left',
'slug' => 'get-more-features',
'name' => __('Get more Features', 'reviews-widgets-for-yelp')
];
}
$tabs[] = [
'place' => 'right',
...
'place' => 'right',
'slug' => 'advanced',
'name' => __('Advanced', 'reviews-widgets-for-yelp')
];
return $tabs;
}
...
return '<a class="ti-btn" href="https://www.trustindex.io/?a=sys&c='. $campaignId .'" target="_blank">'. __('Create a Free Account for More Features', 'reviews-widgets-for-yelp') .'</a>';
}
...
$settingsPageTitle .= mb_strtolower(__('Reviews', 'reviews-widgets-for-yelp'));
}
else {
$settingsPageTitle .= strtolower(__('Reviews', 'reviews-widgets-for-yelp'));
}
$topMenu = false;
...
$platformLink .= 'href="' . admin_url('admin.php?page=' . $this->get_plugin_slug() . '/settings.php&tab=my-reviews') . '">'. __('Review Management', 'reviews-widgets-for-yelp');
}
else {
$platformLink .= 'href="' . admin_url('admin.php?page=' . $this->get_plugin_slug() . '/settings.php') . '">';
if (!$this->is_noreg_linked()) {
/* translators: %s: Platform name */
$platformLink .= sprintf(__('Connect %s', 'reviews-widgets-for-yelp'), $this->platform_name);
}
else {
$platformLink .= __('Create Widget', 'reviews-widgets-for-yelp');
}
}
$platformLink .= '</a>';
$settingsLink = '<a href="' . admin_url('admin.php?page=' . $this->get_plugin_slug() . '/settings.php') . '">' . __('Settings', 'reviews-widgets-for-yelp') . '</a>';
...
{
if (basename($file) === basename($this->plugin_file_path)) {
$meta[] = '<a href="'. admin_url('admin.php?page=' . $this->get_plugin_slug() . '/settings.php&tab=get-more-features') .'">'. __('Get more Features', 'reviews-widgets-for-yelp') . ' →</a>';
$meta[] = '<a href="http://wordpress.org/support/view/plugin-reviews/'. $this->get_plugin_slug() .'" target="_blank" rel="noopener noreferrer">'. __('Rate our plugin', 'reviews-widgets-for-yelp') . ' <span style="color: #F6BB07; font-size: 1.2em; line-height: 1; position: relative; top: 0.05em;">★★★★★</span></a>';
}
return $meta;
}
...
sprintf(__('We have worked a lot on the free "%s" plugin.', 'reviews-widgets-for-yelp'), $this->plugin_name) . '<br />' .
__('If you love our features, please write a review to help us make the plugin even better.', 'reviews-widgets-for-yelp') . '<br />' .
/* translators: %s: Trustindex CEO */
sprintf(__('Thank you. Gabor, %s', 'reviews-widgets-for-yelp'), 'Trustindex CEO'),
],
'not-using-no-connection' => [
'type' => 'warning',
'extra-class' => "",
/* translators: %s: Platform name */
'button-text' => sprintf(__('Create a free %s widget! »', 'reviews-widgets-for-yelp'), $platformName),
'is-closeable' => true,
'hide-on-close' => true,
'hide-on-open' => true,
...
/* translators: %s: Platform name */
'text' => sprintf(__('Display %s reviews on your website.', 'reviews-widgets-for-yelp'), $platformName),
],
'not-using-no-widget' => [
'type' => 'warning',
'extra-class' => "",
/* translators: %s: Platform name */
'button-text' => sprintf(__('Embed the %s reviews widget! »', 'reviews-widgets-for-yelp'), $platformName),
'is-closeable' => true,
'hide-on-close' => true,
'hide-on-open' => true,
...
'redirect' => $defaultRedirect,
/* translators: %s: Platform name */
'text' => sprintf(__('Build trust and display your %s reviews on your website.', 'reviews-widgets-for-yelp'), $platformName),
],
'review-download-available' => [
'type' => 'warning',
'extra-class' => "",
'button-text' => __('Download your latest reviews! »', 'reviews-widgets-for-yelp'),
'is-closeable' => true,
'hide-on-close' => true,
'hide-on-open' => true,
'remind-later-button' => false,
...
/* translators: %s: Platform name */
'text' => sprintf(__('You can update your %s reviews.', 'reviews-widgets-for-yelp'), $platformName),
],
'review-download-finished' => [
'type' => 'warning',
'extra-class' => "",
/* translators: %s: Service name (ChatGPT) */
'button-text' => sprintf(__('Reply with %s! »', 'reviews-widgets-for-yelp'), 'ChatGPT'),
'is-closeable' => true,
'hide-on-close' => true,
'hide-on-open' => true,
'remind-later-button' => false,
'redirect' => '?page='. $this->get_plugin_slug() .'/settings.php&tab=my-reviews',
/* translators: %s: Platform name */
'text' => sprintf(__('Your new %s reviews have been downloaded.', 'reviews-widgets-for-yelp'), $platformName),
],
];
...
.' data-ti-notice-dismiss-text="'. esc_attr(__('Dismiss', 'reviews-widgets-for-yelp')) .'"'
: "";
...
$chosedPlatform = new $className($forcePlatform, $filePath, "do-not-care-14.0", "do-not-care-Widgets for Yelp Reviews", "do-not-care-Yelp");
$chosedPlatform->setNotificationParam('not-using-no-widget', 'active', false);
if (!$chosedPlatform->is_noreg_linked()) {
/* translators: %s: Platform name */
return wp_kses_post($this->frontEndErrorForAdmins(sprintf(__('You have to connect your business (%s)!', 'reviews-widgets-for-yelp'), $forcePlatform)));
...
return self::get_alertbox($type, '<strong>'.$title.'</strong><br />'.$text.'<br /><i style="opacity: 0.65">('. sprintf(__('This message is not be visible to visitors in public mode.') .')</i>', 'reviews-widgets-for-yelp'), false, $dismissUrl);
}
...
<p>' .
'<strong>'. __('ERROR with the following plugin:', 'reviews-widgets-for-yelp') .'</strong> '. $this->plugin_name .'<br /><br />' .
__('CSS file could not saved.', 'reviews-widgets-for-yelp') .' <strong>('. $this->getCssFile() .')</strong> '. __('Your widgets do not display properly!', 'reviews-widgets-for-yelp') . '<br />';
...
if ($fileExists) {
$html .= __('CSS file exists and it is not writeable. Delete the file', 'reviews-widgets-for-yelp');
}
else {
$html .= __('Grant write permissions to upload folder', 'reviews-widgets-for-yelp');
}
$html .= '<br />' .
__('or', 'reviews-widgets-for-yelp') . '<br />' .
/* translators: %s: Advanced page link */
sprintf(__("enable 'CSS internal loading' in the %s page!", 'reviews-widgets-for-yelp'), '<a href="'. admin_url('admin.php?page=' . $this->get_plugin_slug() . '/settings.php&tab=advanced') .'>'. __('Advanced', 'reviews-widgets-for-yelp') .'</a>');
}
echo wp_kses($html, [
'p' => [], 'strong' => [], 'br' => [],
...
return wp_kses_post($this->frontEndErrorForAdmins(__('Free plugin features are unavailable with AMP plugin.', 'reviews-widgets-for-yelp')));
}
...
$text = sprintf(__('Our exclusive "Top Rated" badge is awarded to service providers with a rating of %s and above.', 'reviews-widgets-for-yelp'), self::$topRatedMinimumScore)
.'<br />'
.'<a href="'.admin_url('admin.php?page='.$this->get_plugin_slug().'/settings.php&tab=free-widget-configurator&step=2').'">'.__('Please select another widget', 'reviews-widgets-for-yelp').'.</a>';
...
/* translators: %s: Platform name */
$text = sprintf(__('There are no reviews on your %s platform.', 'reviews-widgets-for-yelp'), ucfirst($this->getShortName()));
...
if (!$reviews) {
return wp_kses_post(self::get_alertbox('error', __('You do not have reviews with the current filters. <br />Change your filters if you would like to display reviews on your page!', 'reviews-widgets-for-yelp')));
}
...
return $this->frontEndErrorForAdmins(__('Could not download the template for the widget.<br />Please reload the page.<br />If the problem persists, please write an email to support@trustindex.io.', 'reviews-widgets-for-yelp') .'<br /><br />'. print_r($response, true));
}
...
if ($this->is_trustindex_connected()): ?>
<?php if ($ti_widgets): ?>
<h2><?php echo esc_html(__('Your saved widgets', 'reviews-widgets-for-yelp')); ?></h2>
...
esc_html(__("You have no widget saved!", 'reviews-widgets-for-yelp')) . " "
. "<a target='_blank' href='" . "https://admin.trustindex.io/" . "widget'>". esc_html(__("Let's go, create amazing widgets for free!", 'reviews-widgets-for-yelp'))."</a>"
)); ?>
<?php endif; ?>
<?php else: ?>
<?php echo wp_kses_post(self::get_alertbox("warning",
esc_html(__("You have not set up your Trustindex account yet!", 'reviews-widgets-for-yelp')) . " "
/* translators: %s: URL */
. wp_kses(sprintf(__("Go to <a href='%s'>plugin setup page</a> to complete the one-step setup guide and enjoy the full functionalization!", 'reviews-widgets-for-yelp'), admin_url('admin.php?page='.$this->get_plugin_slug().'/settings.php&tab=advanced')))
)); ?>
<?php endif;
wp_die();
}
